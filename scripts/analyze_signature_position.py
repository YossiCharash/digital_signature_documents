...
def find_signature_in_pdf(pdf_path: str, signature_image_path: str) -> None:
    ...
    for page_num in range(len(pdf_doc)):
        ...
        print(f"  1. Open the PDF in a PDF viewer")
        print(f"  2. Note the position of the signature stamp")
        print(f"  3. Use the following coordinates (in points, from bottom-left):")
        print(f"     - X: distance from left edge")
        print(f"     - Y: distance from bottom edge")
        print(f"  4. Set in .env:")
        print(f"     SIGNATURE_POSITION_X=<x_value>")
        print(f"     SIGNATURE_POSITION_Y=<y_value>")
        print(f"     SIGNATURE_WIDTH=<width_in_points> (optional, defaults to image size)")
        print(f"     SIGNATURE_HEIGHT=<height_in_points> (optional, defaults to image size)")
